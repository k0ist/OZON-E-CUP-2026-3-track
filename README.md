...
Используется подход **Buy 'Til You Die**:
...
Это показывает высокую важность:

-   частоты покупок;
-   долгосрочного количества заказов;
-   давности активности;
-   ожидаемой будущей активности;
-   истории GMV.
...
Запустить neural-network pipeline.
...
После получения OOF/test predictions выполнить:
...
Feature Engineering выполняется относительно `cutoff`.

Для признаков используется только:

``` python
event_date <= cutoff
```

Target считается отдельно:

``` python
target_start <= event_date <= target_end
```

Таким образом, события из прогнозируемого периода не используются при
...
# 📈 Результаты
  ...
  LightGBM + расширенные признаки                   \~1.72 OOF RMSLE
  Финальный Ensemble                  **1.6720521802 Private RMSLE**

**Важно:** OOF RMSLE и Private Leaderboard RMSLE относятся к разным
выборкам, поэтому сравнивать их напрямую нельзя.
...
Окна до 365 дней позволяют учитывать стабильность поведения
пользователя.

### Поведенческие признаки

Одного количества событий недостаточно --- важны конверсии и отношения
между действиями.

### BTYD

Вероятностная модель повторных покупок предоставляет дополнительные
признаки будущей активности.

### Log-target

`log1p(GMV)` соответствует логарифмической природе RMSLE.

### Ensemble

LightGBM, CatBoost и ResNet дополняют друг друга.

------------------------------------------------------------------------

# 🔮 Возможные улучшения

Следующие направления потенциально могут улучшить результат:
...
``` text
LightGBM ──┐
CatBoost ──┼──> Meta Model ──> Prediction
NN ────────┘
```

Для stacking необходимо использовать только OOF predictions.

## 3. Zero-inflated / Hurdle Model
...
## 6. Более продвинутые Tabular NN
...
и использовать разные модели или post-processing.

------------------------------------------------------------------------

# 🛡️ Reproducibility

Используются фиксированные random seeds.
...
Предсказания нескольких запусков усредняются.
